...
<template>
  <v-container fluid class="py-8">
    <v-row>
      <v-col cols="12">
        <h1 class="text-h4 font-weight-bold mb-8 text-center">Formula 1 Teams 2026</h1>
      </v-col>
    </v-row>
    ...
    <v-row v-else>
      <v-col v-for="team in teams" :key="team._id" cols="12" sm="6" lg="4" xl="3">
        <v-card
        ...
        >
          <v-card-text class="pa-0 h-100 position-relative" style="transform-style: preserve-3d; transition: transform 0.6s;">
            <div
            ...
            >
              ...
              <div class="pa-4 text-white d-flex align-center">
                <v-avatar :color="team.logoColor || 'white'" size="56" class="mr-3">
                  <img :src="getLogoUrl(team.name)" alt="team logo" />
                </v-avatar>
                <h2 class="text-h5 font-weight-bold">{{ team.name }}</h2>
              </div>
            </div>
            ...
            >
              <h3 class="text-h5 mb-4 text-center">{{ team.name }}</h3>
              <div v-if="team.drivers?.length" class="mb-4">
                <strong>Drivers:</strong>
                <ul class="pl-4">
                  <li v-for="driver in team.drivers" :key="driver.full_name">{{ driver.number }} {{ driver.full_name }}</li>
                </ul>
              </div>
